...
void cannot_execute(int);  /* 无法执行，500错误处理函数 */
void not_found(int);   /* 请求的内容不存在 404 */
void unimplemented(int);  /* 仅实现了 GET，POST方法，其他方法错误处理函数 501 */
void error_die(const char *);  /* 错误输出处理函数 */
void headers(int, const char *);  /* 返回http头，服务器成功响应返回200 */

int startup(u_short *); /* 初始化服务器， 开启tcp连接，绑定端口等操作*/
/*  
  每次收到请求，创建一个线程来处理接受到的请求，
  把client_sock转成地址作为参数传入pthread_create 
*/
void accept_request(void *arg); 
void execute_cgi(int, const char *, const char *, const char *);  /* 执行cig脚本 */
/*
  从缓冲区读取一行
...
void serve_file(int, const char *);  /* 处理文件请求，如果不是CGI文件，直接读取文件返回给请求的http客户端 */
...
{
    //socket
    int client = (intptr_t)arg;  //建立连接的 socket描述符
    char buf[1024];   //缓冲区
    int numchars;
    char method[255];
    char url[255];
                 ...
                 const char *method, const char *query_string)
{
    ...
    if (pid == 0)  /* child: CGI script */
    {
        ...
        /* 1 代表stdout，0 代表stdin，将系统标准输出重定向为cgi_output[1] */ 
        //子进程输出 重定向到output管道的 1端
        dup2(cgi_output[1], 1);
        //子进程输入 重定向到input管道的 0端
        dup2(cgi_input[0], 0);
...
{
    int httpd = 0;   //定义服务器socket 描述符
    struct sockaddr_in name;  //此结构体绑定服务器端的ip地址和端口号

    httpd = socket(PF_INET, SOCK_STREAM, 0);   //创建服务器端socket
    if (httpd == -1)
        error_die("socket");
...
int main(void)
{
    int server_sock = -1;    //定义服务器socket 描述符
    u_short port = 0;    // 定义服务器监听端口，随机生成
    int client_sock = -1;   // 定义客户端socket 描述符
    struct sockaddr_in client_name;  //此结构体绑定客户端端的ip地址和端口号
